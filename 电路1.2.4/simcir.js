	...
	var createWorkspace = function(data) {
		...
		! function() {
			...
			$defs.append(createSVGElement('pattern').attr({
				...
				createSVGElement('rect').attr('class', 'simcir-pin-hole').attr({
					x: 0,
					y: 0,
					width: 1,
					height: 1
		...
		var connectors2 = JSON.stringify(getData().connectors);
		

		function download(filename, connectors2) {

			var element = document.createElement('a');
			element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(connectors2));
